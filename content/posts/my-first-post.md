...
throughout the blog, we consider the asymptotic regime where the sample size $n$ grows while the model class is fixed.
...
plotting $\log p(D_n)$ vs $\log n$ reveals $-\lambda$ as the slope. this allows empirical estimation of effective dimension.
...
(evidence slope plots confirming theory.)
